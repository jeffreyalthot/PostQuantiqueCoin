...
PQAlgorithmInfo DevOnlyFallbackCryptoProvider::GetSigningAlgorithmInfo() const { return {"DEV-ONLY-HMAC-SHA256-NON-SECURE",32,32,32,0,0,false}; }
PQAlgorithmInfo DevOnlyFallbackCryptoProvider::GetKemAlgorithmInfo() const { return {"DEV-ONLY-KEM-NON-SECURE",32,32,0,32,32,false}; }
bool DevOnlyFallbackCryptoProvider::IsSigningAlgorithmAllowed(const std::string& algorithm) const { return algorithm==GetSigningAlgorithmInfo().name; }
bool DevOnlyFallbackCryptoProvider::IsKemAlgorithmAllowed(const std::string& algorithm) const { return algorithm==GetKemAlgorithmInfo().name; }
