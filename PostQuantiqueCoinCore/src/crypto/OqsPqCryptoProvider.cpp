...
KemEncapsulation OqsPqCryptoProvider::Encapsulate(const std::vector<uint8_t>& publicKey){ OQS_KEM* kem=OQS_KEM_new(KemAlg()); if(!kem) throw std::runtime_error("ML-KEM unavailable"); if(publicKey.size()!=kem->length_public_key){ OQS_KEM_free(kem); throw std::runtime_error("bad ML-KEM public key size"); } KemEncapsulation e; e.ciphertext.resize(kem->length_ciphertext); e.sharedSecret.resize(kem->length_shared_secret); if(OQS_KEM_encaps(kem,e.ciphertext.data(),e.sharedSecret.data(),publicKey.data())!=OQS_SUCCESS){ OQS_KEM_free(kem); throw std::runtime_error("ML-KEM encapsulation failed"); } OQS_KEM_free(kem); return e; }
std::vector<uint8_t> OqsPqCryptoProvider::Decapsulate(const std::vector<uint8_t>& privateKey,const std::vector<uint8_t>& ciphertext){ OQS_KEM* kem=OQS_KEM_new(KemAlg()); if(!kem) throw std::runtime_error("ML-KEM unavailable"); if(privateKey.size()!=kem->length_secret_key || ciphertext.size()!=kem->length_ciphertext){ OQS_KEM_free(kem); throw std::runtime_error("bad ML-KEM decapsulation input size"); } std::vector<uint8_t> ss(kem->length_shared_secret); if(OQS_KEM_decaps(kem,ss.data(),ciphertext.data(),privateKey.data())!=OQS_SUCCESS){ OQS_KEM_free(kem); throw std::runtime_error("ML-KEM decapsulation failed"); } OQS_KEM_free(kem); return ss; }
PQAlgorithmInfo OqsPqCryptoProvider::GetSigningAlgorithmInfo() const { OQS_SIG* sig=OQS_SIG_new(SigAlg()); if(!sig) return {}; PQAlgorithmInfo i{SigAlg(),sig->length_public_key,sig->length_secret_key,sig->length_signature,0,0,true}; OQS_SIG_free(sig); return i; }
PQAlgorithmInfo OqsPqCryptoProvider::GetKemAlgorithmInfo() const { OQS_KEM* kem=OQS_KEM_new(KemAlg()); if(!kem) return {}; PQAlgorithmInfo i{KemAlg(),kem->length_public_key,kem->length_secret_key,0,kem->length_ciphertext,kem->length_shared_secret,true}; OQS_KEM_free(kem); return i; }
bool OqsPqCryptoProvider::IsSigningAlgorithmAllowed(const std::string& algorithm) const { return algorithm==SigAlg() || algorithm=="ML-DSA-65"; }
bool OqsPqCryptoProvider::IsKemAlgorithmAllowed(const std::string& algorithm) const { return algorithm==KemAlg() || algorithm=="ML-KEM-768"; }
#else
...
PQAlgorithmInfo OqsPqCryptoProvider::GetSigningAlgorithmInfo() const { return {}; }
PQAlgorithmInfo OqsPqCryptoProvider::GetKemAlgorithmInfo() const { return {}; }
bool OqsPqCryptoProvider::IsSigningAlgorithmAllowed(const std::string&) const { return false; }
bool OqsPqCryptoProvider::IsKemAlgorithmAllowed(const std::string&) const { return false; }
#endif
