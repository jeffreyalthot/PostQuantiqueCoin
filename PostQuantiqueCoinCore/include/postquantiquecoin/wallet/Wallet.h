...
private:
    ...
    std::string name_; WalletStorage storage_; PQCryptoProvider* crypto_; bool locked_{true}; SecureBuffer masterKey_; std::vector<uint8_t> walletSalt_; uint32_t kdfIterations_{600000}; std::vector<WalletKey> keys_; std::string unlockPassword_; uint64_t unlockedUntil{0}; uint64_t autoLockSeconds{300};
};
}
